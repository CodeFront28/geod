document.addEventListener("DOMContentLoaded", () => {
  ...
  function initWhyFeatureTabs() {
    ...
    async function startConsoleAnimation() {
      ...
      rows[0].textContent = encryptionSeeds[0];
      rows[5].textContent = "";
      ...
      async function runCycle() {
        rows[0].textContent =
          encryptionSeeds[
            Math.floor(Math.random() * (encryptionSeeds.length - 1))
          ];

        for (const row of middleRows) {
          await typeText(row, randomEncLine(), 16);
          await wait(70);
        }

        await typeText(rows[5], "secure channel established", 20);
      }
